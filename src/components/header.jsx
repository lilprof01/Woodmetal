    ...
    <div className="w-full flex justify-between align-middle bg-[#ffa500] dark:bg-[black] p-4 sm:py-5 sm:px-14 fixed top-0 shadow-md shadow-black z-50">
      ...
      <Link to={"/"}>
        <div className="flex h-full gap-1">
          <div className="flex justify-center">
            <FontAwesomeIcon
              className="text-[#a52a2a] dark:text-[#ffa500] text-2xl h-7"
              icon={faCouch}
            />
          </div>
          <div className="flex align-middle justify-center">
            <h1 className="text-white text-xl sm:text-2xl font-extrabold">
              Woodmetal
              <span className="text-[#a52a2a] dark:text-[#ffa500] opacity-60">
                .
              </span>
            </h1>
          </div>
      ...
      <div className="hidden sm:flex align-middle">
        <ul className="flex align-middle justify-between gap-8 text-[#a52a2a] text-xl">
          <li className="dark:text-white dark:hover:text-[#ffa500] cursor-pointer hover:text-white transition-all">
            <Link to="/">Home</Link>
          </li>
          <li className="dark:text-white dark:hover:text-[#ffa500] cursor-pointer hover:text-white transition-all">
            <Link to="/NewPage">Gallery</Link>
          </li>
          <li className="dark:text-white dark:hover:text-[#ffa500] cursor-pointer hover:text-white transition-all">
            <Link to="/Testpage">About Us</Link>
          </li>
        </ul>
      </div>
      ...
      <div className="hidden sm:flex sm:text-2xl text-lg text-[#a52a2a] dark:text-white align-middle justify-center gap-6 h-full">
        <Link to={"/Wishlist"}>
          <div className="relative">
            <FontAwesomeIcon
              className="h-5 sm:h-7 hover:cursor-pointer"
              icon={faHeart}
            />
      ...
      <div
        onClick={navToggle}
        className={`flex text-white text-3xl font-black align-middle justify-center border border-[#a52a2a] px-1 shadow-sm shadow-black dark:shadow-[#ffa500] sm:hidden hover:cursor-pointer ${
          showNav ? "rotate-180 ease-in-out duration-1000" : "reset"
        }`}
      >
      ...
      <div
        className={`sm:hidden h-[60vh] w-[50vw] fixed top-[70px] right-0 rounded-tl-xl rounded-bl-xl backdrop-blur-md ${
          showNav ? 'flex animate-wiggle' : "nav-hide"
